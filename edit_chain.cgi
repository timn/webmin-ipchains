...
&header($text{'echain_title'}, undef, "edit_chain", 1, undef, undef,
...
print "<TABLE BORDER=2 CELLPADDING=2 CELLSPACING=0 $cb WIDTH=100%>\n<TR>",
      "<TD COLSPAN=13 $tb WIDTH=100%><B>$in{'chain'}</B></TD></TR>\n";
if ($sc) {
 print "<TR><TD COLSPAN=13 $cb><B>$text{'echain_standpol'}: </B>$policy (",
...
for ( my $i=0; $i<@{$chainrules}; $i++) {
 ...
 print "<TD $cb ALIGN=right><A HREF=\"edit_rule.cgi?chain=$in{'chain'}&rule=$line\">$text{'echain_edit'}</A>/",
       ...
       "<A HREF=\"clone_rule.cgi?chain=$in{'chain'}&rule=$line\">$text{'echain_clone'}</A></TD></TR>\n";
}
